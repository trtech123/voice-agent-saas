// apps/voice-engine/src/agent-prompt.ts

/**
 * Dynamic system prompt builder for the Voice Agent SaaS.
 *
 * Replaces the static SYSTEM_PROMPT from FlyingCarpet. Builds a Gemini
 * system instruction from campaign config (script, questions), tenant info
 * (business name), and contact context (name, custom fields).
 */
 ...
 * 1. Identity and role
 * 2. Campaign-specific script (with placeholder replacement)
 * 3. Qualification questions
 * 4. Tool usage instructions
 * 5. Recording consent and compliance
 * 6. Anti-hallucination and identity guard
 * 7. Contact context
 */
...
אם הלקוח מביע התנגדות, מבקש לסיים, או מבקש לחזור אליו במועד אחר - הפסק מיד את השאלות ועבור לשימוש בכלים המתאימים מבלי להתעקש על קבלת תשובות!
${questionsBlock}

## שימוש בכלים
לאחר שאספת תשובות לשאלות ההסמכה, יש ל��שתמש בכלים הבאים:
- **score_lead** — דרג/י את הלקוח לפ�� התשובות (ציון 1-5, סטטוס: hot/warm/cold/not_interested/callback). חובה לקרוא לכלי הזה לפני סיום השיחה.
- **send_whatsapp** — אם הלקוח מעוניין, שלח/י הודעת וואטסאפ עם פרטים נוספים. אמור/י ללקוח שתשלח/י פרטים לוואטסאפ.
- **request_callback** — אם הלקוח מבקש שיחזרו אליו בזמן אחר, חובה להפעיל מיד כלי זה ולהעביר את הזמן המדויק שביקש (למשל "בעוד 5 דקות", "מחר בבוקר"). אין להמשיך בשאלות ההסמכה!
...
- אם הלקוח מבקש שיחזרו אליו במועד מסוים, קרא ל-request_callback מיד עם הזמן המבוקש.
 ...
 */
export function buildRecordingConsentMessage(): string {
 ...
 */
export function buildReconnectInstruction(
  tenantName: string,
  contactName: string | null
): string {
 ...
 */
export function buildIdleNudgeInstruction(): string {
 ...
 */
export function buildHallucinationCorrectionInstruction(): string {
  return "עצור/י. הזכרת מידע מסוים בלי שקראת לכלי. המידע שנתת עלול להיות שגוי. אם הלקוח שאל משהו שדורש בדיקה, אמור/י לו שאת/ה בודק/ת ופנה/י לכלי המתאים. אם אין כלי רלוונטי, אמור/י שתבדוק/י ות��זור/י אליו.";
}
